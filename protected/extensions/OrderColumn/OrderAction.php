...
class OrderAction extends CAction {
    ...
    public function run($pk, $name, $value, $move) {
        ...
        if ($move === 'up' && $model->{$name} != 0 && $order != 1)
        {
            $where=$order-1;
            CActiveRecord::model($this->modelClass)->updateAll(array(''.$name.''=>$order),array('condition'=>''.$name.'='.$where.''));
            $order -= 1;            
        }
        else if ($move === 'down' && $order != $highestOrder)
        {
            $where=$order+1;
            CActiveRecord::model($this->modelClass)->updateAll(array(''.$name.''=>$order),array('condition'=>''.$name.'='.$where.''));
            $order += 1;
