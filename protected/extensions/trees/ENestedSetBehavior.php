...
class ENestedSetBehavior extends CActiveRecordBehavior
{
	 ...
	 * @return CActiveRecord the owner.
	 */
	public function dieselRoots()
	{
		$owner=$this->getOwner();
		$db=$owner->getDbConnection();
		$owner->getDbCriteria()->addCondition($db->quoteColumnName($owner->getTableAlias()).'.'.$db->quoteColumnName($this->leftAttribute).'=1 AND id!=13');
	...
	private function moveNode($target,$key,$levelUp)
	{
        ...
        public function saveFullPath($category)
        {   
            $path='';
            if(!$category->isRoot())
            {
            ...
            $category->path=$path.$category->alias;
            $category->saveNode(); 
            //update all descedants path
            if(!$category->isLeaf())
            {
                $childs = $category->descendants()->findAll();           
                    foreach ($childs as $child)
                    {
                       $this->saveFullPath($child);
                    }
            }
        }
        
        public function updateFullPath($category)
        {
          //  if(!$category->isLeaf())
           // {
                $childs = $category->descendants()->findAll();           
